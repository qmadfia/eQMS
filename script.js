...
// Misalnya, kita sudah memiliki variabel berikut dari menu yang terkait:
let qtyInspectCount = 50; // Ambil jumlah Qty Inspect
let leftClickCount = 8;  // Ambil jumlah Rework Kiri
let rightClickCount = 11; // Ambil jumlah Rework Kanan

// Fungsi untuk menghitung dan menampilkan FTT
function calculateFTT() {
    // Rumus FTT = (Qty Inspect - ((Rework Kiri + Rework Kanan) / 2)) / Qty Inspect
    const totalRework = (leftClickCount + rightClickCount) / 2;
    const ftt = (qtyInspectCount - totalRework) / qtyInspectCount;
    
    // Menampilkan hasil FTT di elemen dengan ID 'ftt-output'
    const fttOutput = document.getElementById('ftt-output');
    fttOutput.textContent = `FTT: ${(ftt * 100).toFixed(2)}%`; // Menampilkan dalam format persen
}

// Menambahkan event listener atau logika untuk menghitung FTT saat ada perubahan pada Qty Inspect, Rework Kiri, atau Rework Kanan
function updateFTT() {
    // Fungsi ini dapat dipanggil setiap kali ada perubahan di Qty Inspect, Rework Kiri, atau Rework Kanan
    calculateFTT();
}

// Update data (misalnya setelah klik Plus/Minus atau input lainnya)
function handleQtyInspectClick() {
    // Logika untuk mengubah qtyInspectCount, bisa ditambahkan disini sesuai dengan Plus/Minus
    qtyInspectCount = parseInt(document.getElementById('qtyInspectOutput').textContent) || 0;
    updateFTT();  // Perbarui FTT setelah Qty Inspect berubah
}

function handleReworkLeftClick() {
    // Logika untuk mengubah leftClickCount, sesuai dengan Plus/Minus
    leftClickCount = parseInt(document.getElementById('left-counter').textContent) || 0;
    updateFTT();  // Perbarui FTT setelah Rework Kiri berubah
}

function handleReworkRightClick() {
    // Logika untuk mengubah rightClickCount, sesuai dengan Plus/Minus
    rightClickCount = parseInt(document.getElementById('right-counter').textContent) || 0;
    updateFTT();  // Perbarui FTT setelah Rework Kanan berubah
}

// Event listeners untuk Qty Inspect dan Rework Kiri/Kanan (sesuaikan dengan elemen Anda)
document.getElementById('qtyInspectOutput').addEventListener('change', handleQtyInspectClick);
document.getElementById('left-counter').addEventListener('change', handleReworkLeftClick);
document.getElementById('right-counter').addEventListener('change', handleReworkRightClick);

// Pemanggilan awal untuk menghitung FTT saat halaman dimuat
updateFTT();
